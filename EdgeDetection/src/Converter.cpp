...
int Converter::RGB2GrayScale(const cv::Mat& sourceImage, cv::Mat& destinationImage)
{
	...
	for (int y = 0; y < heigth; y++, ptrSourceData += sourceWidthStep, ptrDestinationData += destinationWidthStep) {
		...
		for (int x = 0; x < width; x++, ptrSourceRow += sourceChannels, ptrDestinationRow += destinationChannels) {
			...
			ptrDestinationRow[0] = (uchar)value;
		}
	}

	// Lưu ảnh xuống đĩa
	// Utils::saving_image(destinationImage, "result_rgb_to_grayscale.jpg");
...
int Converter::GrayScale2RGB(const cv::Mat& sourceImage, cv::Mat& destinationImage)
{
	...
	// Lưu ảnh xuống đĩa
	// Utils::saving_image(destinationImage, "result_grayscale_to_rgb.jpg");
...
int Converter::RGB2HSV(const cv::Mat& sourceImage, cv::Mat& destinationImage)
{
	...
	for (int y = 0; y < heigth; y++, ptrSourceData += sourceWidthStep, ptrDestinationData += destinationWidthStep) {
		...
		for (int x = 0; x < width; x++, ptrSourceRow += sourceChannels, ptrDestinationRow += destinationChannels) {
			float blueValue = cv::saturate_cast<float>(1.0 * ptrSourceRow[0] / 255);
			float greenValue = cv::saturate_cast<float>(1.0 * ptrSourceRow[1] / 255);
			float redValue = cv::saturate_cast<float>(1.0 * ptrSourceRow[2] / 255);
	...
	// Lưu ảnh xuống đĩa
	//Utils::saving_image(destinationImage, "result_rgb_to_hsv.jpg");
...
int Converter::HSV2RGB(const cv::Mat& sourceImage, cv::Mat& destinationImage)
{
	...
	// Utils::saving_image(destinationImage, "result_hsv_to_rgb.jpg");
...
*/
int Converter::Convert(const cv::Mat& sourceImage, cv::Mat& destinationImage, int type)
{
	// Kiểm trả ảnh đầu vào
	if (!sourceImage.data) {
		std::cout << "[EXCEPTION] Error with input image.\n";
